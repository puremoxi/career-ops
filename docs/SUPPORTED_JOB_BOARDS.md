...
| Eightfold AI | API | Auto-detects `<tenant>.eightfold.ai` boards and reads the public no-auth search API — tries `/api/pcsx/search` first, falling back to `/api/apply/v2/jobs` (SmartApply) on the tenant-specific "PCSX not enabled" 403. Branded hosts (e.g. Netflix's `explore.jobs.netflix.net`) carry no "eightfold" token, so set `provider: eightfold` explicitly plus an `eightfold: { domain: "<company>.com" }` block (Eightfold's internal per-tenant domain key, not derivable from the URL). Paginates 10/page up to `max_pages` (default 150). |
