...
| `check-comp-coverage.mjs` | `advertised_comp` coverage validator over `reports/*.md` — distinguishes `stated-null` (JD genuinely had no comp figure, informational) from `missing-key` (Machine Summary present but the key is absent entirely — schema violation, likely an extraction failure) so the two are never collapsed into one blank cell; zero LLM, zero writes (JSON or `--summary`). Wired into `verify-pipeline.mjs`'s health check. |
