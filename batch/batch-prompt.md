...
1. Posting-level salary from the JD itself (`Advertised (JD)`) — always the anchor.
...
5. Government baseline / sanity check.
6. Static-report fallback.
...
- Static-report fallback: annual salary guides, niche surveys, cached vertical reports
...
Use scraper/aggregator infrastructure only as a transport layer, never as the named authority in the report. Cite the underlying source, not the extraction mechanism.

After the narrative bullets above, include one compact compensation comparison table with these rows, in this order:
...
- If public data is thin or contradictory, write `no reliable public source found` or `insufficient evidence` instead of guessing.
...
- If a row is an inference (`Estimated guaranteed cash`, `Estimated target total comp`, gap rows), mark it conservatively and derive it from the sourced rows above.
...
When a salary figure exists, include the standard comparison table above plus 3-6 HR verification questions tailored to the company type. Do not present advertised compensation as real take-home pay unless the source explicitly supports that interpretation.
